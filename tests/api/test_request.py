...
@allure.title(f'{TITLE_TEXT}: Landing Page')
@pytest.mark.api
...
def test_access_lp(get_url, get_status_code, lp, reg):
    ...
    logger.info(f'Got respcode {respcode} from LP {lp} in region {settings.REGIONS[reg]}')
    with allure.step(f'Код ответа {respcode} LP {lp} в регионе {settings.REGIONS[reg]}'):
        assert respcode == get_status_code


@allure.feature(FEATURE)
@allure.story(STORY_SERV)
@allure.title(f'{TITLE_TEXT}')
@pytest.mark.api
...
def test_get_region(api_client, get_status_code, service, method):
    ...
    if service == settings.SERVICES[2]:
        with allure.step(f'Для сервиса {settings.SERVICES[2]} отправить запрос с доп. параметрами'):
            resp = api_client.get(path='/'.join([service, settings.API_VERSION, method]),
                                  params={'r12': 524613, 'apikey': settings.APIKEY})
    else:
        with allure.step(f'Для сервиса {service} отправить запрос без доп. параметров'):
            resp = api_client.get(path='/'.join([service, settings.API_VERSION, method]),
                                  params={'apikey': settings.APIKEY})
    logger.info(f'Got respcode {resp.status_code} from {service}/{method}')
    with allure.step(f'Код ответа {resp.status_code} от {service}/{method}'):
        assert resp.status_code == get_status_code


@allure.feature(FEATURE)
@allure.story(STORY_SERV)
@allure.title(f'Проверка полученных данных при параметризации запроса')
@pytest.mark.api
...
def test_get_locality(api_client, service, method, locality):
    ...
    with allure.step(f'{REQ_TEXT}{service}/{method}'):
    ...
    logger.info(f'Got response from {service}/{method} and locality {locality}')
    with allure.step(f'Ответ от {service}/{method} не пустой'):
        assert len(resp) != 0


@allure.feature(FEATURE)
@allure.story(STORY_SERV)
@allure.title(f'Проверка кода ответа при параметризации запроса')
@pytest.mark.api
...
def test_get_banners(api_client, get_status_code, service, method, locality):
    ...
    json_data = {"segment": "b2c", "bgroup": "b2c_main", "region": "5200035", "locality": locality,
                 "services": ["noclient"]}
    with allure.step(f'{REQ_TEXT}{service}/{method} и locality code {locality} ({type(locality)})'):
        resp = api_client.post(path='/'.join([service, settings.API_VERSION, method]), headers=headers, json=json_data,
                               params={'apikey': settings.APIKEY})
    logger.info(
        f'Got respcode {resp.status_code} from {service}/{method} and locality code {locality} ({type(locality)})')
    with allure.step(
            f'Код ответа {resp.status_code} от {service}/{method} и locality code {locality} ({type(locality)})'):
...
@allure.feature(FEATURE)
@allure.story(STORY_SERV)
@allure.title(f'Проверка создания заявки при параметризации запроса')
@pytest.mark.api
...
def test_create_order(api_client, city, service, method):
    ...
    with allure.step(f'{REQ_TEXT}{service}/{method} и city code {city} ({type(city)})'):
        resp = api_client.post(path='/'.join([service, settings.API_VERSION, method]), headers=headers, json=json_data,
                               params={'apikey': settings.APIKEY}).json()
    logger.info(f'Got order from {service}/{method} and city code {city} ({type(city)}). Order #{resp["order_id"]}')
    with allure.step(f'Проверка полученного order_id. Номер заявки {resp["order_id"]}'):
        assert isinstance(resp['order_id'], int)


@allure.feature(FEATURE)
@allure.story(STORY_SERV)
@allure.title(f'Валидация структуры ответа')
@pytest.mark.api
@pytest.mark.parametrize('service, method', [(settings.SERVICES[4], settings.METHODS[5])])
def test_validation_schema(api_client, service, method):
    ...
    with allure.step(f'{REQ_TEXT}{service}/{method}'):
        resp = api_client.post(path='/'.join([service, settings.API_VERSION, method]), headers=headers, json=json_data,
                               params={'apikey': settings.APIKEY}).json()
    logger.info(f'Got response from {service}/{method}')
    ...
    logger.info(f'Check schema and structure response from {service}/{method}')
    with allure.step(f'Проверка валидности полученного ответа от {service}/{method}'):
